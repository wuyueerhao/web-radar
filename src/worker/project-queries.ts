...
export async function listProjectSummaries(
  ...
  const rows = await db
    .prepare(
      `SELECT json_object('id',id,'name',json_extract(data,'$.name'),'companyName',json_extract(data,'$.draft.company.name'),'productCount',json_array_length(data,'$.draft.products'),'template',json_extract(data,'$.draft.template'),'coverAssetId',CASE
        WHEN json_extract(data,'$.draft.buildBranch')='template' THEN NULL
        WHEN json_extract(data,'$.draft.buildBranch')='clone' OR (json_extract(data,'$.draft.buildBranch') IS NULL AND json_type(data,'$.draft.cloneConfig')='object')
          THEN (SELECT json_extract(value,'$.assetId') FROM json_each(projects.data,'$.draft.cloneConfig.uiImages') WHERE json_extract(value,'$.role')='home' LIMIT 1)
        ELSE json_extract(data,'$.draft.siteDesign.pages.home.imageAssetId') END,'updatedAt',json_extract(data,'$.updatedAt'),'createdAt',json_extract(data,'$.createdAt'),'offline',json_extract(data,'$.offline'),'publishedReleaseId',json_extract(data,'$.publishedReleaseId')) AS summary FROM projects WHERE ${where}${status !== 'all' ? ` AND (${state})=?` : ''} ORDER BY json_extract(data,'$.updatedAt') DESC, json_extract(data,'$.createdAt') DESC, id DESC LIMIT ? OFFSET ?`,
    )
